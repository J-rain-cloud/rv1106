...
typedef struct rkIVE_LK_CTRL_S {
    RK_U16 u16PtsNum;          /*Number of the feature points,<=500*/
    IVE_U0Q8 u0q8MinEigThr; /*Minimum eigenvalue threshold*/
    RK_U8 u8IterCnt;           /*Maximum iteration times, <=20*/
    IVE_U0Q11 u0q11Eps;     /*Used for exit criteria: dx^2 + dy^2 < u0q11Eps */
...
typedef struct rkIVE_GMM_CTRL_S {
    ...
    RK_U32 u32LastGMMModelNum;
    RK_U32 u32CurGMMModelNum;

    // 1: load models and math info from outer file
    RK_U8 u8LoadModel;
} IVE_GMM_CTRL_S;
...
typedef struct rkIVE_MATCH_BG_MODEL_CTRL_S {
    RK_U8 enMode;  // 0: update 1: training
    RK_U8
        trainNewCodeBook;  // 1: start train a new codebook,need init mean coef
    RK_U8 u8CodeWordNum;   // The cwnum of current training codebook.Value range
                          // is 0~2
    RK_U32 u32CurFrmNum;     /*Current frame timestamp, in frame units */
    RK_U8 u8TrainingTimeThr; /*Potential background replacement time threshold
                             (range: 10 to 100 frames; default: 20) */
    RK_U8 u8TimeThr; /*Potential background replacement time threshold (range:
                     10 to 100 frames; default: 20) */
    RK_U8 u8TimeThrDiv;

    RK_U8 u8DiffMaxThr; /*Maximum of background differential threshold (range: 5
                        to 15; default: 10) */
    RK_U8 u8DiffMinThr; /*Minimum of background differential threshold (range: 5
                        to 15; default: 10) */
    RK_U8 u8FastLearnRate; /*Quick background learning rate (range: 1 to 10;
                           default: 4) */
    RK_U8 u8Alpha;
} IVE_BGM_CTRL_S;
 ...
 * Cast mode
 */
typedef enum rkIVE_CAST_MODE_E {
    IVE_CAST_MODE_BIAS = 0x0,
    IVE_CAST_MODE_ABS = 0x1,
 ...
 * Cast ctrl param
 */
typedef struct rkIVE_CAST_CTRL_S {
    RK_U8 inDataType;
    RK_U8 outDataType;
    RK_S32 minDataVal;
    RK_S32 maxDataVal;
    RK_S32 minNormVal;
    RK_S32 maxNormVal;
    IVE_CAST_MODE_E enMode;
    RK_S32 bias;
    RK_U32 dataScale;
    RK_U8 inDataOverrange;
    // test only
    RK_U8 autoDataScale;
} IVE_CAST_CTRL_S;
