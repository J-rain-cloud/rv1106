...
/* Blob type */
typedef enum rkIVE_BLOB_TYPE_E {
...
dim direction.
**************************************************************************/
typedef struct rkIVE_BLOB_S {
    IVE_BLOB_TYPE_E enType; /* Blob type */
    RK_U32 u32Stride;       /* Stride, a line bytes num */

    RK_U64 u64VirAddr; /* virtual addr */
    RK_U64 u64PhyAddr; /* physical addr */

    RK_U32 u32Num; /* N: frame num or sequence num,correspond to caffe blob's n */
    union {
        struct {
            RK_U32 u32Width;  /* W: frame width, correspond to caffe blob's w */
            RK_U32 u32Height; /* H: frame height, correspond to caffe blob's h */
            RK_U32 u32Chn;    /* C: frame channel,correspond to caffe blob's c */
        } stWhc;
        struct {
            RK_U32 u32Dim;         /* D: vecotr dimension */
            RK_U64 u64VirAddrStep; /* T: virtual adress of   time steps array in each sequence */
        } stSeq;
    } unShape;
